...
Originally developed as research-radar-paper-vault by xuezheng627.
Source: https://github.com/xuezheng627/research-radar-paper-vault

Adapted and extended for nursing-literature-digest:
- vault_category support for mapping keyword groups to display categories
- digestSource tagging for multi-digest vaults
- digest tab UI (Digest → Area → Paper hierarchy)
- Python 3.9 compatibility fixes
- Bidirectional keyword substring matching
"""
...
def load_keyword_groups(config_path: Path | None) -> list[dict[str, Any]]:
    ...
    for group in groups:
        ...
        if label or terms:
            entry: dict[str, Any] = {"label": label or terms[0], "terms": terms}
            vault_category = str(group.get("vault_category", "")).strip()
            if vault_category:
                entry["vault_category"] = vault_category
...
def matching_keyword_groups(paper: dict[str, Any], groups: list[dict[str, Any]]) -> list[str]:
    ...
    for group in groups:
        ...
        label = str(group.get("label", "")).strip()
        # Match if: term in blob, term in hits (exact), hit is substring of term, or term is substring of hit
        def _matches(term: str) -> bool:
            if term in blob or term in hits:
                return True
            return any(hit in term or term in hit for hit in hits)
        if any(_matches(term) for term in terms):
...
def infer_area_and_subtopics(paper: dict[str, Any], groups: list[dict[str, Any]]) -> tuple[str, list[str]]:
    ...
    if matched_groups:
        first_label = matched_groups[0]
        # Use vault_category from the matched group if present, otherwise use label
        area = first_label
        for group in groups:
            if group.get("label") == first_label and group.get("vault_category"):
                area = group["vault_category"]
                break
        subtopics = list(dict.fromkeys([first_label] + keyword_hits + matched_groups[1:] + subjects[:2]))
