...
const els = {
  ...
  digestTabsBar: document.getElementById("digestTabsBar"),
...
const digestOrder = [...new Set(papers.map((p) => p.digestSource).filter(Boolean))];
const digestColorPalette = ["#7cc9bd", "#94bee8", "#f2c879", "#eea7b8", "#afd39b"];
digestOrder.forEach((d, i) => { digestColors[d] = digestColorPalette[i % digestColorPalette.length]; });
...
function filteredPapers() {
  ...
  return papers
    ...
    .filter((paper) => state.digest === "All" || paper.digestSource === state.digest)
...
function renderDigestTabs() {
  if (!els.digestTabsBar) return;
  const allCount = papers.filter(passesImpactThreshold).length;
  const tabs = [
    { label: "Alle Digests", key: "All", count: allCount },
    ...digestOrder.map((d) => ({
      label: d,
      key: d,
      count: papers.filter(passesImpactThreshold).filter((p) => p.digestSource === d).length,
    })),
  ];
  els.digestTabsBar.replaceChildren(
    ...tabs.map(({ label, key, count }) => {
      const btn = document.createElement("button");
      btn.type = "button";
      btn.className = `digest-tab${state.digest === key ? " active" : ""}`;
      if (digestColors[key]) btn.style.setProperty("--digest-color", digestColors[key]);
      btn.innerHTML = `<span class="digest-tab-label">${label}</span><span class="digest-tab-count">${count}</span>`;
...
function renderGroups(items) {
  ...
  const renderAreaSection = (areaItems, area, level = 2) => {
    const section = document.createElement("section");
    const title = document.createElement(`h${level}`);
    title.className = "group-title";
    title.innerHTML = `${formatLabel(area)} <span>${areaItems.length} paper${areaItems.length > 1 ? "s" : ""}</span>`;
    const grid = document.createElement("div");
    grid.className = "paper-grid";
    grid.replaceChildren(...areaItems.map(renderCard));
    section.append(title, grid);
    return section;
  };

  if (state.digest === "All" && digestOrder.length > 1) {
    // Three-level: digest → area → papers
    const digestGroups = new Map();
    for (const paper of items) {
      const ds = paper.digestSource || "Other";
      const area = paper.area || "General Research";
      if (!digestGroups.has(ds)) digestGroups.set(ds, new Map());
      const areaMap = digestGroups.get(ds);
      if (!areaMap.has(area)) areaMap.set(area, []);
      areaMap.get(area).push(paper);
    }

    for (const ds of digestOrder) {
      if (!digestGroups.has(ds)) continue;
      const areaMap = digestGroups.get(ds);
      const digestSection = document.createElement("section");
      digestSection.className = "digest-group";
      const color = digestColors[ds] || "#94bee8";
      digestSection.style.setProperty("--digest-color", color);

      const digestHeader = document.createElement("h2");
      digestHeader.className = "digest-group-title";
      const totalCount = [...areaMap.values()].reduce((s, arr) => s + arr.length, 0);
      digestHeader.innerHTML = `<span class="digest-group-dot"></span>${ds} <span>${totalCount} Paper</span>`;
      digestSection.append(digestHeader);

      for (const [area, areaItems] of [...areaMap.entries()].sort((a, b) => a[0].localeCompare(b[0]))) {
        digestSection.append(renderAreaSection(areaItems, area, 3));
      }
      els.groupedPapers.append(digestSection);
    }
  } else {
    // Single digest selected: just area → papers
    ...
    for (const [area, groupItems] of [...groups.entries()].sort((a, b) => a[0].localeCompare(b[0]))) {
      els.groupedPapers.append(renderAreaSection(groupItems, area, 2));
    }
  }
}
